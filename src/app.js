(() => {
  ...
  // themselves, the same fallback "az" already uses for *its* ties. (The
  // five one-chapter books — Jude, Obadiah, Philemon, 2/3 John — are cited
  // by verse, not chapter, e.g. "Jude 3"; this reads that number as if it
  // were the chapter, which still orders same-book references correctly
  // relative to each other, just not by the true chapter-1 they all share.)
  function canonKey(v) {
    const parsed = parseReference(v.ref);
    const book = parsed && resolveBook(parsed.book);
    return [
      book ? KJV_BOOKS.indexOf(book) : KJV_BOOKS.length,
      parsed ? parsed.chapter : 0,
      parsed && parsed.from !== null ? parsed.from : 0
    ];
  }
