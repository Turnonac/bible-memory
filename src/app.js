(() => {
  ...
  const UNDO_MS = 6000;        // window to undo a removal or a progress reset before it's gone for good
  const ARM_MS = 3000;         // window an arm-then-confirm control (remove, reset) stays armed
  ...
  // The *most recent* attempt only, not an average across v.recent — a verse
  // that just had one bad run should surface immediately, not wait for
  // several more attempts to drag a rolling average down.
  function isStruggling(v) {
    return v.attempts > 0 && v.recent.length > 0 && v.recent[v.recent.length - 1] < STRUGGLE_SCORE;
  }
  ...
  function matchesFilter(v, filter) {
    ...
    if (filter === "struggling") return isStruggling(v);
    return true;
  }
  ...
  function emptyStateMessage(filter, query) {
    const noun = filter === "due" ? "due" : filter === "mastered" ? "mastered" : filter === "new" ? "not-started" : filter === "struggling" ? "struggling" : "";
    if (query) {
    ...
    if (filter === "struggling") return "Nothing needs extra work right now.";
    return "";
  }
  ...
  // Reads a recall score the same way the result panel's own pct color does
  // — verdigris at mastery level, madder below a comfortable 70%, ink between.
  function scoreColor(score) {
    return score >= MASTERY_SCORE ? "var(--verdigris)" : score >= STRUGGLE_SCORE ? "var(--ink)" : "var(--madder)";
  }

  // A sparkline of a verse's own recent scores (v.recent — up to the last 5,
  // oldest to newest). Scaled to its own min/max rather than a fixed 0–100:
  // the absolute number already sits in the text right beside it ("best 92%
  // · 5×"), so the sparkline's job is showing whether recent attempts are
  // trending up or down, and a fixed scale would flatten that into a near-
  // straight line whenever scores cluster near the top, which is most of the
  // time once a verse is past its first few reps.
  function buildSparkline(recent) {
    if (recent.length < 2) return null;
    const w = 40, h = 12, padX = 2, padY = 2;
    const min = Math.min(...recent), max = Math.max(...recent);
    const span = max - min;
    const x = i => padX + (i / (recent.length - 1)) * (w - padX * 2);
    const y = s => span === 0 ? h / 2 : h - padY - ((s - min) / span) * (h - padY * 2);
    const svg = svgEl("svg", {
      class: "spark",
      viewBox: "0 0 " + w + " " + h,
      role: "img",
      "aria-label": "Recent scores, oldest to newest: " + recent.join("%, ") + "%."
    });
    svg.appendChild(svgEl("polyline", {
      points: recent.map((s, i) => x(i) + "," + y(s)).join(" "),
      fill: "none",
    ...
    svg.appendChild(svgEl("circle", {
      cx: x(last), cy: y(recent[last]), r: "1.6", fill: scoreColor(recent[last])
    }));
    return svg;
  }
  ...
  // chapter — the same fact lookupReference() reads off the decompressed KJV
  // index (via "no chapter 2" for the book) when resolving a typed
  // reference. That index isn't available synchronously where sorting runs,
  // but which five books these are isn't data that needs verifying against
  // the text in the first place — it's a fixed structural fact about the
  // canon — so canonKey() below just names them directly.
  const SINGLE_CHAPTER_BOOKS = new Set(["Obadiah", "Philemon", "2 John", "3 John", "Jude"]);

  // Where a reference falls in the 66-book canon, for "Canon order" below.
  // KJV_BOOKS (src/kjv-data.js) already holds that book sequence for the
  // reference lookup elsewhere in this file, so this needs no lookup of its
  // own — just the same parseReference()/resolveBook() "Add by reference"
  // already uses to make sense of a typed reference. A reference that
  // doesn't parse as "Book chapter[:verse]" (a custom verse with no numeric
  // reference, say) or names no real book sorts after every canonical one —
  // there's nowhere in the canon to place it — tied by reference text among
  // themselves, the same fallback "az" already uses for *its* ties.
  function canonKey(v) {
    const parsed = parseReference(v.ref);
    const book = parsed && resolveBook(parsed.book);
    let chapter = parsed ? parsed.chapter : 0;
    let verse = parsed && parsed.from !== null ? parsed.from : 0;
    // A bare "Jude 3" parses with chapter=3, verse=0 — normalize it to
    // chapter 1, verse 3 (what it actually cites) so it sorts correctly
    // against an explicit "Jude 1:4" for the same book, rather than a
    // fictitious "chapter 3" outranking a real chapter 1.
    if (parsed && book && parsed.from === null && SINGLE_CHAPTER_BOOKS.has(book)) {
      verse = chapter;
      chapter = 1;
    }
    return [book ? KJV_BOOKS.indexOf(book) : KJV_BOOKS.length, chapter, verse];
  }
  ...
  // not canonical book order — a search aid, not a Bible index; "canon" is
  // that Bible-index ordering, via canonKey() above.
  function sortDeck(list, sortKey) {
    ...
    if (sortKey === "canon") {
      return list.slice().sort((a, b) => {
        const ka = canonKey(a), kb = canonKey(b);
    ...
    shown.forEach(v => {
      ...
      if (isMastered(v)) {
        ...
        svg.appendChild(svgEl("path", {
          d: "M2.5 8.5 6 12l7.5-8", "stroke-linecap": "round", "stroke-linejoin": "round"
        }));
        ref.appendChild(svg);
      }
      ...
      const left = el("span");
      if (v.attempts) {
        left.appendChild(document.createTextNode("best " + v.best + "% · " + v.attempts + "×"));
        const spark = buildSparkline(v.recent);
        if (spark) left.appendChild(spark);
      } else {
        left.textContent = "not yet recited";
      }
      ...
      // with no attempts is already at the state reset would put it in.
      if (v.attempts) {
        const reset = el("button", "reset-progress");
        reset.type = "button";
        reset.textContent = "reset";
        reset.setAttribute("aria-label", "Reset progress on " + v.ref);
  ...
  // Both a removal and a progress reset destroy real practice history with no
  // way back through the deck itself, so each gets the same short grace
  // window rather than trusting its own arm-then-confirm click alone. One
  // banner, one pending slot, whichever kind of action last claimed it.
  function armUndo(kind, message, payload) {
    if (pendingUndo) clearTimeout(pendingUndo.timer);
    pendingUndo = Object.assign({ kind: kind, timer: setTimeout(clearPendingUndo, UNDO_MS) }, payload);
    $("undoMsg").textContent = message;
    $("undoBanner").hidden = false;
    // The button the reader just clicked was just removed from the DOM (or,
    // for a reset, changed out from under them) — hand focus to the one
    // control that can undo what happened to it.
    $("undoBtn").focus();
  }

  function offerUndo(verse, index, wasActive, wasNextDue) {
    armUndo("remove", "Removed " + verse.ref + ".",
      { verse: verse, index: index, wasActive: wasActive, wasNextDue: wasNextDue });
  }

  function offerResetUndo(verse, snapshot) {
    armUndo("reset", "Reset progress on " + verse.ref + ".", { id: verse.id, snapshot: snapshot });
  }
  ...
  function undoRemove() {
    if (!pendingUndo || pendingUndo.kind !== "remove") return;
  ...
  // Puts a verse back to "not yet recited" — attempts, best, recent scores,
  // and the whole SM-2 schedule — while keeping it, its reference, and its
  // text exactly as they are. For a reader picking a verse back up after a
  // long gap and wanting a genuinely fresh start on it, rather than deleting
  // and re-adding it (which would also require retyping a custom verse's
  // text and re-resolving a starter verse's KJV lookup).
  function resetVerse(id) {
    const v = state.verses.find(x => x.id === id);
    if (!v || !v.attempts) return;
    const snapshot = {
      attempts: v.attempts, best: v.best, last: v.last, recent: v.recent.slice(),
      ease: v.ease, reps: v.reps, interval: v.interval, due: v.due
    };
    Object.assign(v, { attempts: 0, best: 0, last: null, recent: [], ease: EASE_START, reps: 0, interval: 0, due: null });
    // A reset doesn't touch the verse's text, so peeked/hideOrder (indices
    // into that text) stay valid — unlike an edit, there's no need to clear
    ...
    save();
    renderAll();
    offerResetUndo(v, snapshot);
  }

  function undoReset() {
    if (!pendingUndo || pendingUndo.kind !== "reset") return;
  ...
  function runCheck() {
    ...
    $("pct").style.color = scoreColor(res.score);
